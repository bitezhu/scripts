...
for panel in panellist:
	...
	for targetfile in targetfiles:
		...
		if not os.path.exists(unchangedFile):
			continue
		samplename = os.path.basename(tumorBam).split("_",1)[0]
		os.system("cp %s ."%unchangedFile)
		print unchangedFile
		os.system("python /home/zhusihui/scripts/sam2pileup.py %s %s > %s_SNP_data"%(tumorBam,unchangedFile,samplename))
		os.system("python SNPfilter.py sampleCollection.xls %s_SNP_data %s"%(samplename,os.path.basename(unchangedFile)))
		'''
		tmpdict={}
